...
class Array4d : public std::vector<REAL> // inherit from the STL vector
{
public:
	...
	Array4d( const size_type size, const REAL defaultvalue = 0) 
	: data_(size*size*size*size, defaultvalue)
	, n_per_dim_(size)
	{
	...
	Array4d(Vector<number_type> q) 
	: data_(q)
	, n_per_dim_(sqrt(sqrt(q.size())))
	{
	...
	// Access operators
	// A(i, j)
	number_type & operator()(const size_type t, const size_type x, const size_type y, const size_type z)
	{
		assert( t < n_per_dim_ && x < n_per_dim_ && y < n_per_dim_ && z < n_per_dim_ );
		return data_[get_index(t, x, y, z)];
	}
	const number_type & operator()(const size_type t, const size_type x, const size_type y, const size_type z) const
	{
		assert( t < n_per_dim_ && x < n_per_dim_ && y < n_per_dim_ && z < n_per_dim_ );
		return data_[get_index(t, x, y, z)];
	}
	...
	/* Convert vector index to space-time quadruple */
	void get_position(const size_type & index, size_type & t, size_type & x, size_type & y, size_type & z)
	{
		size_type i = index;
		t = i % n_per_dim_;
		i = i / n_per_dim_;
	...
	/* Convert space-time quadruple to vector index */
	size_type get_index(const size_type & t, const size_type & x, const size_type & y, const size_type & z)
	{
		return t + x*n_per_dim_ + y*n_per_dim_*n_per_dim_ + z*n_per_dim_*n_per_dim_*n_per_dim_;
	}


	/* Get average value at time index t */
	number_type average_at(size_type t)
	{
		...
		for (size_type x = 0; x < n_per_dim_; ++x)
		{
			...
			{
				for (size_type z = 0; z < n_per_dim_; ++z)
				{
					Gamma_t += (*this)(t, x, y, z);
				}
			}
		}

		return Gamma_t/n_per_dim_/n_per_dim_/n_per_dim_;
	}
